...
void Desktop::EnableHook(IntPtr windowHandle) {
	...
	_messageHookHandle = SetWindowsHookEx(WH_CALLWNDPROC, (HOOKPROC)GetProcAddress(hinstDLL, "MessageHookProc"), hinstDLL, tid);
}
...
Object^ Desktop::GetEventHandler(Type^ eventHandlerType, Object^ instance) {
	...
	try {
		...
		DynamicMethod^ dm = gcnew DynamicMethod(fnName, EventHandler::typeid, params, Desktop::typeid);
